...
def create_walkers(walking_time: int, number_of_usual_walkers: int,
                   number_of_fast_walkers: int, number_of_running_walkers: int) -> list:
    """
...
def calculate_the_path(number_of_steps: int) -> tuple:
    ...
    walkers_locations = np.random.randint(low=0, high=number_of_steps,
    ...
    starting_position = walkers_locations[0, :]
    ...
    # store last position
    ending_position = walkers_locations[-1, :]
...
def plot_the_paths(list_of_walkers: list, outputfilename: str) -> None:
    ...
    for walker in enumerate(list_of_walkers):
        ...
        if walker[1].walking_speed == 1:
            walker_type = 'usual'
        if walker[1].walking_speed == 2:
            walker_type = 'fast'
        if walker[1].walking_speed == 4:
            walker_type = 'running'
        start_coordinates = [walker[1].starting_position[0], walker[1].starting_position[1]]
        end_coordinates = [walker[1].ending_position[0], walker[1].ending_position[1]]
        ...
        if np.absolute(distance[0]) and np.absolute(distance[1]) >= 50:
            axis[row, column].plot([start_coordinates[0], end_coordinates[0]],
            ...
            axis[row, column].scatter(walker[1].ending_position[0],
                                      walker[1].ending_position[1], label='Endposition')
            axis[row, column].legend()
            axis[row, column].set_title(f'Walker {walker[0] + 1} took a plane, '
...
class UsualWalker:
    ...
    def __init__(self, walking_time: int):
        ...
        self.x_coordinates, self.y_coordinates, self.starting_position, \
            self.ending_position = calculate_the_path(self.walking_time * self.walking_speed)
...
def main():
    ...
    try:
        ...
        outputfilename = r"C:\Users\yanni\Desktop\test_walker.png"

        number_of_walkers = number_of_usual_walkers\
    ...
    scene = create_walkers(walking_time,
